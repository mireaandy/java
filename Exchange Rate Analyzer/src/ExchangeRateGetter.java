...
public class ExchangeRateGetter {
    ...
    public String getRate(String day, Rates values) throws IOException, InterruptedException, Exception {
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("https://api.exchangeratesapi.io/" + day
                + "?symbols=" + values.getSymbol() + "&base=" + values.getBase())).build();
        ...
        return responeJsonObject.getJSONObject("rates").get(values.getSymbol()).toString() + "_"
                + responeJsonObject.get("date").toString();
    }
